...
def wrong_first_dataloader(model, tokenized_datasets, num_cls = 4, current_epoch = 1, device = torch.device("cpu")):
    '''
    save and load model --
    #torch.save(model,'bert-cls.bert')   
    #new_model = torch.load(model_file) 
...
def train_model(model,learning_rate, best_model,train_dataloader, valid_dataloader,test_dataloader,
    ...
    for w, epoch in enumerate(range(num_epochs)):
        ...
        # wrong_first_dataloader(model, tokenized_datasets, num_cls=4, device = torch.device("cpu")):
        counter = 0
